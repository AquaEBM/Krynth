...
impl Oscillator {
    ...
    #[inline]
    fn get_sample_from_table(&self, table: &BandlimitedWaveTables, frame: [usize ; 2]) -> StereoSample {
        StereoSample {
...
impl WTOscVoice {
    ...
    fn update_num_unison_voices(&mut self, [new, _]: [usize ; 2]) {
        ...
        if new > current {
            self.oscillators.extend(iter::repeat_with(
                || Oscillator::new(random::<f32>() * PHASE_RANGE)
            ).take(new - current));
        } else {
    ...
    fn update_phases(&mut self, detune_range: StereoSample) {
        ...
        self.oscillators[odd..].array_chunks_mut().for_each(|[osc1, osc2]| {
    ...
    #[inline]
    fn get_sample_from_table(&self, table: &BandlimitedWaveTables, frame: [usize ; 2], detune: StereoSample) -> StereoSample {

        let pan = (detune * StereoSample::splat(0.5)).sqrt();

        let odd = self.oscillators.len() & 1;
        let accumulator = if odd == 0 { ZERO_SAMPLE } else {
            self.oscillators[0].get_sample_from_table(table, frame)
        };

        self.oscillators[odd..].array_chunks().map(|[voice1, voice2]| {
